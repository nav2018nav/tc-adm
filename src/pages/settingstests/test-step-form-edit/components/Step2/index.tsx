...
const Step2: React.FC<Step2Props> = (props) => {
  ...
  const { test } = props;
  //const data11 = test.physical;
  let main_physical_arr = [];
  let main_practical_arr = [];

  for (var i = test?.physical?.length - 1; i >= 0; i--) {
    main_physical_arr.push({
      key: test.physical[i],
      name: "",
      editable: true,
      isNew: true,
  ...
  const [data1, setData] = useState(main_physical_arr);
  //console.log(data1);
  window.new_Data = main_physical_arr;

  for (var i = test?.practical?.length - 1; i >= 0; i--) {
    main_practical_arr.push({
      key: test.practical[i],
      name: "",
      editable: true,
      isNew: true,
      id: test.practical[i],
  ...
  const [data2, setData2] = useState(main_practical_arr);
  window.new_Data2 = main_practical_arr;
  ...
  useEffect(() => {
    dispatch({
      type: "testFormAndstepFormEdit/fetchQuestionsPractical",
    });
  }, [1]);

  useEffect(() => {
    dispatch({
      type: "testFormAndstepFormEdit/fetchQuestionsPhysical",
    });
  }, [1]);

  // const [data1, setData1] = useState(item1);
  ...
  const physical_columns = [
    {
      ...
      render: (text: string, record: TableFormDateType) => {
        if (record.editable) {
          // console.log(record);
          //console.log(test.physical);
          let selected =
            test?.physical.indexOf(record.key) != -1 ? record.key : "";
          //console.log(questionsListPhysical);
          ...
          return (
            <span>
              ...
              >
                <Option value=""> <FormattedMessage id="Please.select.question" defaultMessage='Please select question type'/></Option>
                {questionsListPhysical?.map((item) =>
                  test.physical.indexOf(item.id) != -1 ? (
  ...
  const practical_columns = [
    {
      ...
      render: (text: string, record: TableFormDateType) => {
        if (record.editable) {
          let selected =
            test.practical.indexOf(record.key) != -1 ? record.key : "";
          return (
            <span>
              <Select
                placeholder={intl.formatMessage({
      ...
      <>
                  ...
                  return (
                    <Form.Item
                      ...
                      label={intl.formatMessage({
                        ...
                        defaultMessage: "Test Length (Minutes)",
                      })}
                      rules={[
                        {
                          required: true,
        ...
        <Row gutter={24}>
          ...
          <Col span={12}>
            ...
            >
              <h4>
                <b>
                  <FormattedMessage id="select_type_physical.step.list.TestName" />
                </b>{" "}
              </h4>
        ...
        <Row gutter={24}>
          ...
          <Col span={12}>
            ...
            >
              <h4>
                 <b><FormattedMessage id="select_type_practicle.step.list.TestName" /> :
                </b>{" "}
              </h4>
              <Table<TableFormDateType>
  ...
  return (
    <>
      <Form {...formItemLayout} form={form} initialValues={test}>
        {renderContent()}
      </Form>
    </>
  );
...
export default connect(
  ({
    ...
    const { questionsListPractical } = testFormAndstepFormEdit;
    const { questionsListPhysical } = testFormAndstepFormEdit;

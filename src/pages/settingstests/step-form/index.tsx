...
const StepForm: React.FC<StepFormProps> = ({ current }) => {
  ...
  let done = localStorage.getItem('step');
  //console.log(done);
  ...
  return (
    <PageContainer content="" onBack={() => window.history.back()} title={<FormattedMessage id="que.step1.msg.title.Questions" defaultMessage=""/>}>
      <Card bordered={false}>
        <>
...
export default connect(({ questionFormAndstepForm }: { questionFormAndstepForm: StateType }) => ({
  current: questionFormAndstepForm.current,
}))(StepForm);
